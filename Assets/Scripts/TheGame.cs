...
namespace FarmWolffun
{
    ...
    public class TheGame : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            //Spawn worker
            foreach (KeyValuePair<string, SaveWorkerData> elem in pdata.workers)
            {
                if (elem.Value.spawned)
                {
                    Worker worker = Worker.Spawn(elem.Key, elem.Value);
                    worker?.transform.SetParent(spawn_parent.transform);
                }
            }
