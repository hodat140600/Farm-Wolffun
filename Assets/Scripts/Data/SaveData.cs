...
namespace FarmWolffun
{
    ...
    public class SaveData
    {
        ...
        public Dictionary<string, SaveWorkerData> workers = new Dictionary<string, SaveWorkerData>(); //Workers
        ...
        public Dictionary<AttributeType, float> attributes = new Dictionary<AttributeType, float>(); //Generic worker area attributes, not specific to characters
        ...
        public void FixData()
        {
            ...
            if (workers == null)
                workers = new Dictionary<string, SaveWorkerData>();
            if (npcs == null)
            ...
            foreach (KeyValuePair<string, SaveWorkerData> worker in workers)
                worker.Value.FixData();
        }
        ...
        public SaveWorkerData GetWorker(string uid)
        {
            if (workers.ContainsKey(uid))
                return workers[uid];
            SaveWorkerData worker = new SaveWorkerData();
            workers[uid] = worker;
            return worker;
        }

        public bool HasWorker(string uid)
        {
            return workers.ContainsKey(uid);
        }
