...
namespace FarmWolffun
{
    public enum AttackGroup
    {
        ...
        Ally=10,   //Will be attacked automatically by enemies, cant be attacked by workers
        Enemy=20,  //Will be attacked automatically by workers
    ...
    public class Destructible : MonoBehaviour
    {
        ...
        public bool IsAlly()
        {
            return attack_group == AttackGroup.Ally;
        }
        ...
        public static Destructible GetNearest(AttackGroup group, Vector3 pos, float range = 999f)
        {
            Destructible nearest = null;
            float min_dist = range;
            foreach (Destructible destruct in destruct_list)
            {
                ...
                {
                    float dist = (destruct.transform.position - pos).magnitude;
                    if (dist < min_dist && destruct.attack_group == group)
                    {
                        min_dist = dist;
                        nearest = destruct;
                    }
