...
namespace FarmWolffun
{
    ...
    /// <summary>
    /// Zones are areas defined by the player where worker will go work automatically. 
    /// 
    /// </summary>
    ...
    public class Zone : MonoBehaviour
    {
        ...
        public Gatherable GetNearestGatherable(Vector3 pos, float range = 999f)
        {
            float min_dist = range;
        ...
        public static Zone GetNearestUnassigned(Vector3 pos, float range = 999f)
        {
            ...
            foreach (Zone zone in zone_list)
            {
                ...
                if (zone.CanBeGathered() && !zone.Workable.IsFullyAssigned() && dist < min_dist)
                {
                    min_dist = dist;
                    nearest = zone;
                }
