...
namespace Farm
{
    ...
    public class TechManager : MonoBehaviour
    {
        ...
        public int GetTechBonusInt(BonusType type, Selectable receiver = null, Selectable target = null, CraftData itarget = null)
        {
            return Mathf.RoundToInt(GetTechBonus(type, receiver, target, itarget));
        }

        public float GetTechBonus(BonusType type, Selectable receiver = null, Selectable target = null, CraftData itarget = null)
        {
            float bonus = 0f;
            foreach (KeyValuePair<string, int> pair in SaveData.Get().tech_completed)
            {
                ...
                if (itech && itech.effect == type)
                {
                    ...
                    bool can_target = is_any || itech.CanTarget(target) || itech.CanTarget(itarget);
