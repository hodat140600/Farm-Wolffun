...
namespace Farm
{
    ...
    public class CharacterEquip : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            foreach (ItemData item in GetEquippedItems())
            {
        ...
        public float GetEquipBonus(BonusType bonus, Selectable target = null, CraftData itarget =null)
        {
            float value = 0;
            foreach (ItemEquipData item in GetItems())
            {
                bool is_any = target == null && itarget == null;
                bool is_valid_select = target != null && target.HasGroup(item.bonus_target);
        ...
        public bool HasTool(GroupData group)
        {
