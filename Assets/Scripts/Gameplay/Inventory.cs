...
namespace Farm
{
    public class Inventory : MonoBehaviour
    {
        ...
        public void TransferGlobalItems()
        {
            /*InventoryData ginventory = GetGlobalData();
            if (ginventory == SData)
                return;

            Dictionary<string, int> tranfered = new Dictionary<string, int>();
            foreach (KeyValuePair<string, int> pair in SData.items)
            {
                ItemData idata = ItemData.Get(pair.Key);
                if (idata != null && idata.global)
                {
                    ginventory.AddItem(idata.id, pair.Value);
                    if (tranfered.ContainsKey(idata.id))
                        tranfered[idata.id] += pair.Value;
                    else
                        tranfered[idata.id] = pair.Value;
                }
            }
            foreach (KeyValuePair<string, int> pair in tranfered)
            {
                SData.AddItem(pair.Key, -pair.Value);
            }*/
        }
        ...
        {
            int bonus = TechManager.Get().GetTechBonusInt(BonusType.InventorySize, Selectable);
            return (cargo_max > 0 && !global) ? (cargo_max + bonus) : int.MaxValue;
        }
