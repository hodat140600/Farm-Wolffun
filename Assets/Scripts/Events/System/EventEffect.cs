...
namespace Farm.Events
{
    public enum NarrativeEffectType
    {
        ...
        AddWorkerAttribute=30,
        SetWorkerAttribute = 32,
    ...
    public class EventEffect : MonoBehaviour
    {
        ...
        public void Trigger()
        {
            ...
            if (type == NarrativeEffectType.AddWorkerAttribute)
            {
                WorkerData cdata = (WorkerData)value_data;
                GroupData gdata = value_group;
                if (cdata != null)
                {
                    Worker worker = Worker.Get((WorkerData)value_data);
                    if (worker?.Attributes != null)
                        worker.Attributes.AddAttribute(value_attr, value_float);
                }
                else if (gdata != null)
                {
                    foreach (Worker worker in Worker.GetAllGroup(gdata))
                    {
                        if (worker?.Attributes != null)
                            worker.Attributes.AddAttribute(value_attr, value_float);
                    }
            ...
            if (type == NarrativeEffectType.SetWorkerAttribute)
            {
                WorkerData cdata = (WorkerData)value_data;
                GroupData gdata = value_group;
                if (cdata != null)
                {
                    Worker worker = Worker.Get((WorkerData)value_data);
                    if (worker?.Attributes != null)
                        worker.Attributes.SetAttribute(value_attr, value_float);
                }
                else if (gdata != null)
                {
                    foreach (Worker worker in Worker.GetAllGroup(gdata))
