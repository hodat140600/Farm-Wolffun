...
namespace FarmWolffun.Events
{
    ...
    public class EventCondition : MonoBehaviour
    {
        ...
        public bool IsMet()
        {
            ...
            if (type == NarrativeConditionType.WorkerAttribute)
            {
                Worker worker = Worker.Get((WorkerData)value_data);
                float i1 = worker?.Attributes != null ? worker.Attributes.GetAttributeValue(value_attr) : 0f;
            ...
            {
                float i1 = WorkerManager.Get().GetAverageAttribute(value_attr);
                float i2 = value_float;
                condition_met = CompareFloat(oper, i1, i2);
            }
